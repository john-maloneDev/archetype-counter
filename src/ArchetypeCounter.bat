...
$ArchetypeCounterForm.Add_Load({
    ...
    $MenuItem_Exit.add_Click({  
        ...
        $SetConfigSettings = "$Global:CounterWorkingDir\stored\Config_Settings.txt"; $GetConfigSettings = [IO.File]::ReadAllLines("$SetConfigSettings")
...
$ArchetypeCounterForm.Add_Shown({
    ...
    if ($ReShadeAlertDialog -match "Show") { $ReShadeinPokeMMO = ''; if (Test-Path "$PokeMMOWorkingDir\jre\bin\ReShade.ini") { $ReShadeinPokeMMO = '(ReShade being used)'; . SetDialogTransparentBackground; $TaskDialogReShade = [Ookii.Dialogs.WinForms.TaskDialog]::new(); $TaskDialogReShade.WindowTitle = "Archetype Counter"; $TaskDialogReShade.WindowIcon = $ArchetypeCounterSystrayIcon; $TaskDialogReShade.MainIcon = [Ookii.Dialogs.WinForms.TaskDialogIcon]::Warning; $TaskDialogReShade.MainInstruction = "ReShade Detection"; $TaskDialogReShade.Content = "Using post processing filters like ReShade can produce inaccurate count results. Please disable ReShade before reporting any bugs."; $TaskDialogReShade.VerificationText = 'Do not show this message again'; $TaskDialogReShade.AllowDialogCancellation = $true; $TaskDialogReShade.Buttons.Add('Ok'); $TaskDialogReShade.ShowDialog($ArchetypeCounterForm); . RemoveDialogTransparentBackground }; if ($TaskDialogReShade.IsVerificationChecked -eq $true) { $GetConfigSettings = $GetConfigSettings -replace "ReShade_Alert_Dialog=.*", "ReShade_Alert_Dialog=Ignore"; [IO.File]::WriteAllLines($SetConfigSettings, $GetConfigSettings) } }
            ...
            if ($Global:PokeMMO_hwnd -ne $null -and $PokeMMOWindowState -notmatch 'Minimized') {
                ...
                if ($GetPixelColor -match '\b('+($PixelMatchArray -join '|')+')\b') { 
                    ...
                    foreach ($OCRCapture in $OCRCaptured) {
                        ...
                        $SetConfigProfileEncountered = "$Global:CounterWorkingDir\stored\$($Script:SyncHashTable.GetProfile)\Config_$($Script:SyncHashTable.GetProfile)_Encountered.txt";
                        $GetConfigProfileEncountered = [IO.File]::ReadAllLines($SetConfigProfileEncountered);
                        $CapturedPokemon = $GetPokeConfig | Where-Object { $_ -match "\b$OCRCapture\b" } | Select -First 1;
                        $CapturedPokemonID = $CapturedPokemon -Replace '[^0-9]','' -Replace ' ', '';
                        $CapturedPokemonName = $CapturedPokemon -Replace '[0-9]','' -Replace '-', '' -Replace ' ', ''
                        ...
                        if (!([string]::IsNullOrEmpty($CapturedPokemon)) -or !([string]::IsNullOrWhitespace($CapturedPokemon))) { 
                            ...
                            if ($GetConfigProfileEncountered | Where-Object { $_ -match "\b$OCRCapture\b" }) {
                                ...
                                $OCRCaptureEncountered = $GetConfigProfileEncountered | Where-Object { $_ -match "\b$OCRCapture\b" };
                                $OCRCaptureEncounteredNumber = ([regex]::Matches($OCRCaptureEncountered, "\((\d+)\)").Groups[1].Value) -join '';
                                $OCRCaptureEncounteredNumber = [int]$OCRCaptureEncounteredNumber + [int]1;
                                $GetConfigProfileEncountered = $GetConfigProfileEncountered | ForEach-Object { 
                                    if ($_ -match "\b$OCRCapture\b") { "#$CapturedPokemonID - $OCRCapture ($OCRCaptureEncounteredNumber)" 
                                } else { $_ } };
                                [IO.File]::WriteAllLines($SetConfigProfileEncountered, $GetConfigProfileEncountered)
